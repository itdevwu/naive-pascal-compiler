...
void Visitor::visitTypeDefinition(pascalParser::TypeDefinitionContext *context)
{
    ...
    if (auto typeContext = dynamic_cast<pascalParser::Type_Context *>(context))
    {
        auto type = visitType_(typeContext);
    ...
    else if (auto functionTypeContext = dynamic_cast<pascalParser::FunctionTypeContext *>(context))
    {
        // visitFunctionType(functionTypeContext);
    }
    else if (auto procedureTypeContext = dynamic_cast<pascalParser::ProcedureTypeContext *>(context))
    {
...
llvm::Type *Visitor::visitType_(pascalParser::Type_Context *context)
{
    if(auto simpleType = dynamic_cast<pascalParser::SimpleTypeContext *>(context)){
        return visitSimpleType(simpleType);
        
    }
    else if(auto structuredType = dynamic_cast<pascalParser::StructuredTypeContext *>(context)){

    }
    else if(auto pointerType = dynamic_cast<pascalParser::PointerTypeContext *>(context)){
...
{
    if(auto ident = dynamic_cast<pascalParser::IdentifierContext *>(context)){
        ...
        else if (auto charContext = context->STRING())
        {
            if (isVar)
...
void Visitor::visitProcedureOrFunctionDeclaration(pascalParser::ProcedureOrFunctionDeclarationContext *context){
    // 判断走是哪一个声明
    if (auto procedureDeclarationContext = dynamic_cast<pascalParser::ProcedureDeclarationContext *>(context)){
        visitProcedureDeclaration(procedureDeclarationContext);
    }
    else if (auto functionDeclarationContext = dynamic_cast<pascalParser::FunctionDeclarationContext *>(context))
    {
...
{
    if (auto parameterGroupContext = dynamic_cast<pascalParser::ParameterGroupContext *>(context))
    {
...
{
    if (auto assignmentStatementContext = dynamic_cast<pascalParser::AssignmentStatementContext *>(context)){
        visitAssignmentStatement(assignmentStatementContext);
    }
    else if (auto procedureStatementContext = dynamic_cast<pascalParser::ProcedureStatementContext *>(context))
        visitProcedureStatement(procedureStatementContext);
    else if (auto emptyStatementContext = dynamic_cast<pascalParser::EmptyStatement_Context *>(context))
        visitEmptyStatement_(emptyStatementContext);
    else if (auto gotoStatementContext = dynamic_cast<pascalParser::GotoStatementContext *>(context))
...
llvm::Value *Visitor::visitExpression(pascalParser::ExpressionContext *context) {
    ...
    if (auto relationalOperatorContent = dynamic_cast<pascalParser::RelationaloperatorContext *>(context)) {
...
llvm::Value *Visitor::visitSimpleExpression(pascalParser::SimpleExpressionContext *context) {
    ...
    if(auto addictiveOperatorContext = dynamic_cast<pascalParser::AdditiveoperatorContext *>(context)) {
...
llvm::Value *Visitor::visitTerm(pascalParser::TermContext *context) {
    ...
    if(auto multiplicativeOperatorContext = dynamic_cast<pascalParser::MultiplicativeoperatorContext *>(context)) {
...
llvm::Value *Visitor::visitFactor(pascalParser::FactorContext *context, int flag, float flag_fp, llvm::ConstantInt* flag_v, llvm::Constant* flag_v_fp) {
    if(auto variableContext = dynamic_cast<pascalParser::VariableContext *>(context)) {
    ...
    else if(auto expressionContext = dynamic_cast<pascalParser::ExpressionContext *>(context)) {
    ...
    else if(auto functionDesignatorContext = dynamic_cast<pascalParser::FunctionDesignatorContext *>(context)) {
    ...
    else if(auto unsignedConstant = dynamic_cast<pascalParser::UnsignedConstantContext *>(context)) {
    ...
    else if(auto bool_Context = dynamic_cast<pascalParser::Bool_Context *>(context)) {
    ...
    else if(auto set_Context = dynamic_cast<pascalParser::Set_Context *>(context)) {//Todo
...
llvm::Value *Visitor::visitUnsignedConstant(pascalParser::UnsignedConstantContext *context) {
    if(auto unsignedNumberContext = dynamic_cast<pascalParser::UnsignedNumberContext *>(context)) {
        return visitUnsignedNumber(context -> unsignedNumber());
    }
    else if(auto constantChrContext = dynamic_cast<pascalParser::ConstantChrContext *>(context)) {
        return visitConstantChr(context -> constantChr());
    }
    else if(auto stringContext = dynamic_cast<pascalParser::StringContext *>(context)) {
        auto v = visitString(stringContext);
        auto v_words = v.substr(1, v.length() - 2);
        return builder.CreateGlobalStringPtr(v_words);
    }
    else if(context -> NIL()) {//Todo

    }
}
*/
/*
llvm::Value* Visitor::visitConstantChr(pascalParser::ConstantChrContext *context) {//Todo
...
void Visitor::visitStructuredStatement(pascalParser::StructuredStatementContext *context, llvm::Function *function) {
    if (auto compoundStatementContext = dynamic_cast<pascalParser::CompoundStatementContext *>(context -> compoundStatement())) {
        visitCompoundStatement(compoundStatementContext, function);
    }
    else if(auto repetitiveStatementContext = dynamic_cast<pascalParser::RepetetiveStatementContext *>(context ->repetetiveStatement())) {
        visitRepetetiveStatement(repetitiveStatementContext, function);
    }
    else if(auto conditionalStatementContext = dynamic_cast<pascalParser::ConditionalStatementContext *>(context -> conditionalStatement())) {
        visitConditionalStatement(conditionalStatementContext, function);
    }
    else if(auto withStatementContext = dynamic_cast<pascalParser::WithStatementContext *>(context -> withStatement())) {
...
void Visitor::visitRepetetiveStatement(pascalParser::RepetetiveStatementContext *context, llvm::Function *function) {
    if(auto forStatementContext = dynamic_cast<pascalParser::ForStatementContext *>(context -> forStatement())) {
        visitForStatement(forStatementContext, function);
    }
    else if(auto whileStatementContext = dynamic_cast<pascalParser::WhileStatementContext *>(context -> whileStatement())) {
        visitWhileStatement(whileStatementContext, function);
    }
    else if(auto repeatStatementContext = dynamic_cast<pascalParser::RepeatStatementContext *>(context -> repeatStatement())) {
...
void Visitor::visitForStatement(pascalParser::ForStatementContext *context, llvm::Function *function) {
    ...
    if (auto simpleStatementContext = dynamic_cast<pascalParser::SimpleStatementContext *>(context->statement()))
        visitSimpleStatement(simpleStatementContext, function);
    else if (auto structuredStatementContext = dynamic_cast<pascalParser::StructuredStatementContext *>(context->statement()))
...
void Visitor::visitConditionalStatement(pascalParser::ConditionalStatementContext *context, llvm::Function *function) {
    if(auto ifStatementContext = dynamic_cast<pascalParser::ConditionalStatementContext *>(context -> ifStatement())) {
        visitConditionalStatement(ifStatementContext, function);
    }
    else {
...
void Visitor::visitUnlabelledStatement(pascalParser::UnlabelledStatementContext *context, llvm::Function *function) {
    if(auto simpleStatementContext = dynamic_cast<pascalParser::SimpleStatementContext *>(context -> simpleStatement())) {
        visitSimpleStatement(simpleStatementContext, function);
    }
    else if(auto structuredStatementContext = dynamic_cast<pascalParser::StructuredStatementContext *>(context -> structuredStatement())) {
        visitStructuredStatement(structuredStatementContext, function);
    }
}
...
llvm::Type *Visitor::visitArrayType(pascalParser::ArrayTypeContext *context, std::vector<std::string> idList) {
    auto ranges = visitTypeList(context->typeList());
    int eleNum = 1;
    ...
    if(context -> LBRACK() || context -> LBRACK2()) {
        ...
        // 得到数组的类型，并作为返回值
        auto type = visitComponentType(context->componentType(), idList, eleNum);
...
llvm::Type *Visitor::visitComponentType(pascalParser::ComponentTypeContext *context, std::vector<std::string> idList, int eleNum) {
    return visitType_(context->type_(), idList, eleNum);
}
llvm::Type *Visitor::visitType_(pascalParser::Type_Context *context, std::vector<std::string> idList, int eleNum) {
    if(context->simpleType()){
        auto type = visitSimpleType(context->simpleType());
        return type;
    }
    else if(context->structuredType()){
        auto type = visitStructuredType(context->structuredType(), idList);
        return type;
    }
    else
    {
...
std::vector<int> Visitor::visitTypeList(pascalParser::TypeListContext *context) {
